...
.OUTPUTS
    Hashtable
#>
function Get-PSFHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-PSFConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-PSFStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-PSFUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-PSFUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-PSFStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-PSFKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-PSFRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-PSFECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-PSFCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
function Get-PSFRSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "PSFRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-PSFECDSASignature {
    param(
...
function Get-PSFCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-PSFKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
